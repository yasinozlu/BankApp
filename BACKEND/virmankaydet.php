...
session_start();
    ...
    $bakiyesorgu = $baglan->query("SELECT hesapBakiye FROM tblMusteriHesap  WHERE hesapNumarasi = '{$hesapNumarasi}' and hesapEkNumarasi='{$gonderenEkNumarasi}' ")->fetchAll(PDO::FETCH_ASSOC);
   
     if($bakiyesorgu->$hesapBakiye>$gonderilenbakiye)
     {
		...
		$query2 = $baglan->prepare("UPDATE tblMusteriHesap SET 
         ...
         $query3->bindParam(':gonderenEkNumarasi', $gonderenEkNumarasi);
             ...
             if ( $insert && $insert2 && $insert3  )
			{
			    ...
			    echo json_encode("ok");
			    echo json_encode($bakiyesorgu->$hesapBakiye);
			}
			else
			{
